...
class HealthManager:
    ...
    async def _handle_health_result(self, health_result: Dict[str, Any]) -> None:
        ...
        if overall_status in ["unhealthy", "critical"]:
            ...
            if self.consecutive_failures >= self.max_consecutive_failures and self.recovery_enabled:
                await self._trigger_recovery(health_result)
        else:
            if self.consecutive_failures > 0:
                self.logger.info(
                    "Health restored after failures", previous_failures=self.consecutive_failures
                )
            self.consecutive_failures = 0
    ...
    async def _trigger_recovery(self, health_result: Dict[str, Any]) -> None:
        ...
        try:
            ...
            log_server_event(
                self.logger, "recovery_complete", "Automatic recovery procedures completed"
            )

        except Exception as e:
            log_server_event(self.logger, "recovery_fail", f"Automatic recovery failed: {e}")
...
async def restart_server_recovery(health_result: Dict[str, Any]) -> None:
    ...
    log_server_event(logger, "recovery_restart", "Attempting server restart for health recovery")
