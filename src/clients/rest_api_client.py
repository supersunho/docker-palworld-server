    ...
    async def _make_request_with_retry(
        ...
        for attempt in range(retry_count + 1):
            try:
                ...
                log_api_call(
                    self.logger, endpoint, 0, duration_ms, attempt=attempt + 1, error=str(e)
                )
        ...
        try:
            async with self.session.request(**kwargs) as response:
                ...
                if response.status == 200:
                    ...
                    self.logger.warning(
                        "API request failed", status=response.status, response=response_text[:200]
                    )
                    return None
