            ...
            async with self.discord_notifier as notifier:
                if event.event_type == PlayerEventType.JOINED:
                    await notifier.notify_player_join(
                        event.player_name, event.player_count, language=self._language
                    )
            ...
            async with self.discord_notifier as notifier:
                if event.event_type == ServerEventType.STATUS_CHANGED:
                    if "restarted" in event.message.lower():
                        ...
                        await notifier.notify_server_stop(event.message, language=self._language)
                    ...
                    await notifier.notify_error(
                        f"Server performance issue: {event.message}", language=self._language
                    )
