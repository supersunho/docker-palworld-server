...
class IdleRestartManager:
    ...
    def __init__(self, config: PalworldConfig, player_monitor, process_manager):
        ...
        if idle_config:
            ...
            try:
                self.idle_minutes = (
                    int(idle_config.idle_minutes) if hasattr(idle_config, "idle_minutes") else 30
                )
    ...
    async def stop_monitoring(self) -> None:
        ...
        log_server_event(self.logger, "idle_monitoring_stop", "Idle monitoring stopped")
    ...
    async def _handle_zero_players(self, current_time: float) -> None:
        ...
        if minutes_elapsed > 0 and minutes_elapsed % 5 == 0:
            ...
            if remaining_minutes > 0:
                self.logger.debug(
                    f"Server idle for {minutes_elapsed}m " f"({remaining_minutes}m until action)"
                )
    ...
    async def _handle_paused_state(self, current_time: float) -> None:
        ...
        if paused_duration >= max_pause_seconds:
            self.logger.warning(f"Server paused for {max_pause_hours}h — performing full restart")
    ...
    async def _trigger_idle_action(self) -> None:
        ...
        await self._send_discord_notification(
            self.mode, f"Server idle for {self.idle_minutes}m — triggering {self.mode}"
        )
        ...
        try:
            ...
            if success:
                ...
                log_server_event(
                    self.logger, "idle_action_fail", f"Idle action ({self.mode}) failed"
                )
    ...
    async def _force_resume(self) -> None:
        ...
        if resume_ok:
            ...
            self.logger.warning("Force resume failed -- server did not resume successfully")
