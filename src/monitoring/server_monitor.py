...
class ServerMonitor:
    ...
    def remove_event_callback(
        self, event_type: ServerEventType, callback: Callable[[ServerEvent], Awaitable[None]]
    ) -> bool:
        ...
        if current_status.is_running != self._last_status.is_running:
            if current_status.is_running:
                event = ServerEvent(
                    ...
                    details={"pid": current_status.pid, "previous_status": "stopped"},
                    timestamp=time.time(),
                )
            else:
                event = ServerEvent(
            ...
            and self._last_status.is_running
            and current_status.pid != self._last_status.pid
        ):
            event = ServerEvent(
                event_type=ServerEventType.STATUS_CHANGED,
