...
class INIToYAMLConverter:
    ...
    def _read_file_with_encoding_detection(
        self, file_path: Path, max_bytes: Optional[int] = None
    ) -> str:
    ...
    def _infer_data_type_and_format(self, value: str) -> Tuple[str, Any]:
        ...
        if value.lower() in ["true", "false"]:
            return (
                f"${{{self._convert_to_env_var_name('temp')}:{value.lower()}}}",
                value.lower() == "true",
            )
            ...
            if category_settings:
                ...
                for setting_name in sorted(category_settings):
                    try:
                        ...
                        yaml_format = yaml_format.replace(
                            self._convert_to_env_var_name("temp"), env_var_name
                        )
