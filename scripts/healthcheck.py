...
class HealthChecker:
    ...
    def __init__(self):
        self.rest_api_enabled = os.getenv("REST_API_ENABLED", "true").lower() in (
            "true",
            "1",
            "yes",
        )
            ...
            for proc in psutil.process_iter(
                ...
                try:
                    ...
                    if "PalServer" in proc_info.get("name", "") or (
                        ...
                        and any("PalServer" in str(cmd) for cmd in proc_info.get("cmdline", []))
                    ):
        ...
        try:
            ...
            if not port_check["success"]:
                return HealthCheckResult(
                    ...
                    details={"port": self.rcon_port, "error": port_check["error"]},
                    response_time_ms=(time.time() - start_time) * 1000,
                    timestamp=time.time(),
                )
            ...
            if rcon_test["success"]:
                return HealthCheckResult(
                    ...
                    details={"port": self.rcon_port, "test_command": "Info"},
                    response_time_ms=response_time,
                    timestamp=time.time(),
                )
            else:
                return HealthCheckResult(
                    ...
                    details={"port": self.rcon_port, "error": rcon_test["error"]},
                    response_time_ms=response_time,
                    timestamp=time.time(),
                )
    ...
    async def _check_rcon_port(self) -> Dict[str, Any]:
        ...
        try:
            reader, writer = await asyncio.wait_for(
                asyncio.open_connection(self.rcon_host, self.rcon_port), timeout=5
            )
            writer.close()
    ...
    async def _test_rcon_command(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                process.kill()
                await process.wait()
                return {"success": False, "response": None, "error": "RCON command timeout"}
            ...
            return {"success": False, "response": None, "error": f"RCON test error: {str(e)}"}
...
async def async_main():
    ...
    try:
        ...
        if format_json:
            print(json.dumps({"error": f"Health check failed: {str(e)}", "timestamp": time.time()}))
        else:
